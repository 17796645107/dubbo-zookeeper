				...
				<div class="product">
					...
					<c:forEach items="${pageInfo.list}" var="goods">
						<li>
							...
							<div class="proImg">
								<a href="#">
									<img width="165px" height="183px" class="lazy" src="js/lazyload/grey.gif" data-original="images/${goods.goodsImg }" alt="">
								</a>
							</div>
							<div class="proTxt">
								<p class="p_name"><a href="javascript:void(0)">${goods.goodsName }</a>
								<p class="num">已售出${goods.goodsNum }件</p>
								<p>
									<strong>
										￥<fmt:formatNumber value="${goods.goodsPrice }" pattern=".00"></fmt:formatNumber>

									</strong>
									<strong>
										￥<fmt:formatNumber value="${goods.goodsPrice * 1.5 }" pattern=".00"></fmt:formatNumber>
									</strong>
								</p>
							</div>
						</li>
				...
				<div class="footNum">
					<ul>
						<c:choose>
							...
							<c:otherwise>
								<li class="pre">
									<a target="_self" href="/GoodsWEB/goods/listGoods?page=${pageInfo.pageNum-1}">上一页</a>
								</li>
							</c:otherwise>
						</c:choose>
						<c:forEach begin="1" end="${pageInfo.pages}" step="1" var="index">
							<c:choose>
								<c:when test="${pageInfo.pageNum eq index}">
									<li class="num current">${index}</li>
								</c:when>
								<c:otherwise>
									<li class="num"><a target="_self" href="/GoodsWEB/goods/listGoods?page=${index}">${index}</a></li>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						<c:choose>
							...
							<c:otherwise>
								<li class="last">
								<a target="_self" href="/GoodsWEB/goods/listGoods?page=${pageInfo.pageNum + 1}">下一页</a></li>
							</c:otherwise>
						</c:choose>
